...
impl BehaviourAdaptor for PanCamera {
    fn update(
        ...
        if input_query.is_key_down(InputKey::Left) {
            interaction.pan_camera_by(Position::new(-amount, 0.0));
        }

        if input_query.is_key_down(InputKey::Right) {
            interaction.pan_camera_by(Position::new(amount, 0.0));
        }

        if input_query.is_key_down(InputKey::Up) {
            interaction.pan_camera_by(Position::new(0.0, -amount));
        }

        if input_query.is_key_down(InputKey::Down) {
            interaction.pan_camera_by(Position::new(0.0, amount));
        }
    }
}
